...
def min_list(data: List[Tuple]) -> List[Tuple]:
    ...
    while poi not in hull:
        ...
        next_poi, angle = smallest_counter_clockwise_angle(
            points=shifted_data, previous_angle=angle
        )
...
def smallest_counter_clockwise_angle(
    points: List[Tuple], previous_angle: float
) -> Tuple:
    """
    ...
    for idx, (angle, distance) in enumerate(angles_and_distances):
        if angle > previous_angle:
            if (angle < smallest_angle) or (
                (angle == smallest_angle) & (distance > biggest_distance)
            ):
                next_point_idx = idx
